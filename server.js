...
function tick() {
  ...
  for (const code in rooms) {
    ...
    if (allDead && Object.keys(room.enemies).length > 0) {
...
function fireArrowVolley(room, boss) {
  for (let i = 0; i < 5; i++) {
    setTimeout(() => {
      ...
      const ev = { type: 'arrowEvent', startX: boss.x, startY: boss.y, targetX: target.player.x, targetY: target.player.y, targetPlayerId: target.player.id, dmg: arrowDmg };
      broadcast(room, ev);
      setTimeout(() => {
        if (!rooms[room.code]) return;
        const p = room.players[target.player.id];
        if (p && p.hp > 0) {
