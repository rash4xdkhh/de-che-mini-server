...
function broadcastState(room) {
  ...
  for (const id in room.players) {
    ...
    playersOut[id] = { id, name: p.name, x: p.x, y: p.y, hp: p.hp, maxHp: p.maxHp, wave: room.wave, level: p.level, swordsLevel: p.swordsLevel, swordsUpgraded: p.swordsUpgraded, shieldActive: p.shieldActive, shieldUpgraded: p.shieldUpgraded, giants: p.giants || [] };
  }
  const enemiesOut = {};
  for (const id in room.enemies) {
    const e = room.enemies[id];
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'join') {
      ...
      room.players[playerId] = {
        ...
        swordsLevel: 0, swordsUpgraded: false, shieldActive: false, shieldUpgraded: false, giants: [],
      };
    ...
    if (msg.type === 'move') {
      ...
      if (typeof msg.shieldActive === 'boolean') me.shieldActive = msg.shieldActive;
      if (typeof msg.shieldUpgraded === 'boolean') me.shieldUpgraded = msg.shieldUpgraded;
